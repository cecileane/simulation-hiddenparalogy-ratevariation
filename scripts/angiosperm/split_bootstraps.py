...
After `iqtree -S alignments --prefix iqtree/loci -B 1000 -wbtl`, IQ-TREE
writes
...
Usage (from the data set folder):  python3 split_bootstraps.py iqtree 1000
"""
import os
import re
import sys
...
assert len(ml) == len(names), \
    f"{len(ml)} ML trees vs {len(names)} partitions"
assert len(boot) == B * len(names), \
    f"{len(boot)} bootstrap trees, expected {B * len(names)}"
