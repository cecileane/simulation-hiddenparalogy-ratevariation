...
Usage:
julia scripts/simulation_postprocess.jl \
  --dup_rate 0.0 --loss_rate 0.0 --ratevar N --n_reps 100
...
# set up paramname_root based on given parameter values
# -> see utilities.jl for details
...
# Only for n_inds = 1
#-----------------------------------------------#
function create_simphy_species_rf_summary(
    ...
    2. Classifies gene trees based on events
       (hidden paralogy, gene loss, nothing)
    ...
    Returns:
        Vector{Tuple}: Collected RF data as tuples
    ...
    for simulation_rep in 1:n_reps
        rep_output_folder = setup_rep_output_folders(
            folder_path_list, simulation_rep, "")
        rf_fname = "RF_btw_simphy_vs_true_species_tree_" *
                   "$simulation_rep.txt"
        rf_fname_noF = "RF_btw_simphy_vs_true_species_tree_noF_" *
                       "$simulation_rep.txt"
        rf_simphy_true_file = joinpath(rep_output_folder, rf_fname)
        rf_simphy_true_file_noF = joinpath(rep_output_folder, rf_fname_noF)
        ...
        if isfile(rf_simphy_true_file)
            ...
            println("Found $(length(rf_lines)) lines in RF file " *
                    "for rep $simulation_rep")
            ...
            for line in rf_lines
                if !isempty(strip(line))
                    ...
                    if length(parts) >= 2
                        ...
                        # Get noF score or NaN
                        rf_score_noF = get(
                            rf_noF_dict, gene_file_path, NaN)

                        # Determine classification based on file path
                        gene_duplication_and_loss_flag =
                            gene_file_path in
                            gene_duplication_and_loss_trees ? "Y" : "N"
                        gene_loss_only_flag =
                            gene_file_path in
                            gene_loss_only_trees ? "Y" : "N"
                        nothing_flag = (
                            gene_duplication_and_loss_flag == "N" &&
                            gene_loss_only_flag == "N") ? "Y" : "N"
    ...
    println("Total collected RF data points: " *
            "$(length(simphy_species_csv_data))")
...
1. gene_tree_file_path: gene tree file paths across all reps
2. num_taxa_gene_tree: Number of taxa in each gene tree
3. avg_internal_branch_length_locus_tree: Avg internal BL
4. avg_internal_branch_length_gene_tree: Avg internal BL
5. RF_true_genetrees_vs_species_tree: RF dist true vs sptree
6. RF_true_vs_estimated_genetrees: RF dist true vs estimated
7. gene_duplication_and_loss_or_not: duplication and loss?
8. gene_loss_only_or_not: gene loss only?
9. nothing_or_not: no changes?
10. false_HP: false hidden paralogy?
11. weak_HP: weak hidden paralogy?
12. strong_HP: strong hidden paralogy?

What we want to compute in the resulting summary csv file:
Appended to simulation_$paramname_root_summary.csv

# Anything related to number of taxa:
1. mean_num_taxa_all_gene_trees: Mean taxa all gene trees
2. mean_num_taxa_loss_only_gene_trees: Mean taxa loss-only
3. mean_num_taxa_gene_duplication_and_loss_gene_trees:
   Mean taxa for dup-and-loss genes
4. mean_num_taxa_nothing_gene_trees: Mean taxa, no changes
5. mean_num_taxa_false_HP: Mean taxa, false HP genes
6. mean_num_taxa_weak_HP: Mean taxa, weak HP genes
7. mean_num_taxa_strong_HP: Mean taxa, strong HP genes

# Anything about internal branch length:
8. mean_avg_internal_branch_length_locus_trees_all_genes:
   -> Mean avg internal BL for locus trees in each rep
9. mean_avg_internal_branch_length_gene_trees_all_genes:
   -> Mean avg internal BL for gene trees in each rep
10. mean_avg_internal_branch_length_locus_trees_loss_only_gene_trees:
    -> Mean avg internal BL locus trees, loss-only genes
11. mean_avg_internal_branch_length_gene_trees_loss_only_gene_trees:
    -> Mean avg internal BL gene trees, loss-only genes
12. mean_avg_internal_branch_length_locus_trees_
    gene_duplication_and_loss_gene_trees:
    -> Mean avg internal BL locus trees, dup+loss genes
13. mean_avg_internal_branch_length_gene_trees_
    gene_duplication_and_loss_gene_trees:
    -> Mean avg internal BL gene trees, dup+loss genes
14. mean_avg_internal_branch_length_locus_trees_nothing_gene_trees:
    -> Mean avg internal BL locus trees, no-change genes
15. mean_avg_internal_branch_length_gene_trees_nothing_gene_trees:
    -> Mean avg internal BL gene trees, no-change genes
16. mean_avg_internal_branch_length_locus_trees_false_HP:
    -> Mean avg internal BL locus trees, false HP genes
17. mean_avg_internal_branch_length_gene_trees_false_HP:
    -> Mean avg internal BL gene trees, false HP genes
18. mean_avg_internal_branch_length_locus_trees_weak_HP:
    -> Mean avg internal BL locus trees, weak HP genes
19. mean_avg_internal_branch_length_gene_trees_weak_HP:
    -> Mean avg internal BL gene trees, weak HP genes
20. mean_avg_internal_branch_length_locus_trees_strong_HP:
    -> Mean avg internal BL locus trees, strong HP genes
21. mean_avg_internal_branch_length_gene_trees_strong_HP:
    -> Mean avg internal BL gene trees, strong HP genes

# RF distance between true genetrees vs species tree
22. mean_RF_true_genetrees_vs_species_tree_all_genes:
    -> Mean RF dist true gene trees vs sptree, all genes
23. mean_RF_true_genetrees_vs_species_tree_loss_only_gene_trees:
    -> Mean RF dist true gene trees vs sptree, loss-only
24. mean_RF_true_genetrees_vs_species_tree_
    gene_duplication_and_loss_gene_trees:
    -> Mean RF dist true gene trees vs sptree, dup+loss
25. mean_RF_true_genetrees_vs_species_tree_nothing_gene_trees:
    -> Mean RF dist true gene trees vs sptree, no changes
26. mean_RF_true_genetrees_vs_species_tree_false_HP:
    -> Mean RF dist true gene trees vs sptree, false HP
27. mean_RF_true_genetrees_vs_species_tree_weak_HP:
    -> Mean RF dist true gene trees vs sptree, weak HP
28. mean_RF_true_genetrees_vs_species_tree_strong_HP:
    -> Mean RF dist true gene trees vs sptree, strong HP

# RF distance between true genetrees vs estimated genetrees
29. mean_RF_true_vs_estimated_genetrees_all_genes:
    -> Mean RF dist true vs estimated genetrees, all genes
30. mean_RF_true_vs_estimated_genetrees_loss_only_gene_trees:
    -> Mean RF dist true vs estimated genetrees, loss-only
31. mean_RF_true_vs_estimated_genetrees_
    gene_duplication_and_loss_gene_trees:
    -> Mean RF dist true vs estimated genetrees, dup+loss
32. mean_RF_true_vs_estimated_genetrees_nothing_gene_trees:
    -> Mean RF dist true vs estimated genetrees, no changes
33. mean_RF_true_vs_estimated_genetrees_false_HP:
    -> Mean RF dist true vs estimated genetrees, false HP
34. mean_RF_true_vs_estimated_genetrees_weak_HP:
    -> Mean RF dist true vs estimated genetrees, weak HP
35. mean_RF_true_vs_estimated_genetrees_strong_HP:
    -> Mean RF dist true vs estimated genetrees, strong HP

Those new columns will be appended to the
simulation_$paramname_root_summary.csv file
=#
"""
...
function summarize_rf_distances(input_csv_file::String, 
    ...
    df_rf[!, :RepID] = [
        parse(Int,
            match(r"rep(\d+)", row.gene_tree_file_path).captures[1])
...
"""
function fill_rep_with_missing!(df::DataFrame, rep_id::Int,
...
calculate_rep_statistics!:
    Calculate all summary statistics for a single replicate
    and update the summary dataframe.
"""
function calculate_rep_statistics!(df_summary::DataFrame, 
    ...
    dup_and_loss = rep_data[
        rep_data.gene_duplication_and_loss_or_not .== "Y", :]
    ...
    df_summary[row_idx, :mean_num_taxa_all_gene_trees] =
        mean(rep_data.num_taxa_gene_tree)
    df_summary[row_idx, :mean_num_taxa_loss_only_gene_trees] =
        safe_mean(loss_only.num_taxa_gene_tree)
    df_summary[row_idx, col_ntaxa_dup_loss] =
        safe_mean(dup_and_loss.num_taxa_gene_tree)
    df_summary[row_idx, col_ntaxa_nothing] =
        safe_mean(nothing_trees.num_taxa_gene_tree)
    df_summary[row_idx, :mean_num_taxa_false_HP] =
        safe_mean(false_hp.num_taxa_gene_tree)
    df_summary[row_idx, :mean_num_taxa_weak_HP] =
        safe_mean(weak_hp.num_taxa_gene_tree)
    df_summary[row_idx, :mean_num_taxa_strong_HP] =
        safe_mean(strong_hp.num_taxa_gene_tree)

    # Calculate mean internal branch lengths - locus trees
    df_summary[row_idx, col_lbl_all] =
        mean(rep_data.avg_internal_branch_length_locus_tree)
    df_summary[row_idx, col_lbl_loss] =
        safe_mean(loss_only.avg_internal_branch_length_locus_tree)
    df_summary[row_idx, col_lbl_dup_loss] =
        safe_mean(dup_and_loss.avg_internal_branch_length_locus_tree)
    df_summary[row_idx, col_lbl_nothing] =
        safe_mean(nothing_trees.avg_internal_branch_length_locus_tree)
    df_summary[row_idx, col_lbl_falseHP] =
        safe_mean(false_hp.avg_internal_branch_length_locus_tree)
    df_summary[row_idx, col_lbl_weakHP] =
        safe_mean(weak_hp.avg_internal_branch_length_locus_tree)
    df_summary[row_idx, col_lbl_strongHP] =
        safe_mean(strong_hp.avg_internal_branch_length_locus_tree)

    # Calculate mean internal branch lengths - gene trees
    df_summary[row_idx, col_gbl_all] =
        mean(rep_data.avg_internal_branch_length_gene_tree)
    df_summary[row_idx, col_gbl_loss] =
        safe_mean(loss_only.avg_internal_branch_length_gene_tree)
    df_summary[row_idx, col_gbl_dup_loss] =
        safe_mean(dup_and_loss.avg_internal_branch_length_gene_tree)
    df_summary[row_idx, col_gbl_nothing] =
        safe_mean(nothing_trees.avg_internal_branch_length_gene_tree)
    df_summary[row_idx, col_gbl_falseHP] =
        safe_mean(false_hp.avg_internal_branch_length_gene_tree)
    df_summary[row_idx, col_gbl_weakHP] =
        safe_mean(weak_hp.avg_internal_branch_length_gene_tree)
    df_summary[row_idx, col_gbl_strongHP] =
        safe_mean(strong_hp.avg_internal_branch_length_gene_tree)

    # Calculate mean RF distance: true gene trees vs species tree
    df_summary[row_idx, col_rf_sp_all] =
        mean(rep_data.RF_true_genetrees_vs_species_tree)
    df_summary[row_idx, col_rf_sp_loss] =
        safe_mean(loss_only.RF_true_genetrees_vs_species_tree)
    df_summary[row_idx, col_rf_sp_dup_loss] =
        safe_mean(dup_and_loss.RF_true_genetrees_vs_species_tree)
    df_summary[row_idx, col_rf_sp_nothing] =
        safe_mean(nothing_trees.RF_true_genetrees_vs_species_tree)
    df_summary[row_idx, col_rf_sp_falseHP] =
        safe_mean(false_hp.RF_true_genetrees_vs_species_tree)
    df_summary[row_idx, col_rf_sp_weakHP] =
        safe_mean(weak_hp.RF_true_genetrees_vs_species_tree)
    df_summary[row_idx, col_rf_sp_strongHP] =
        safe_mean(strong_hp.RF_true_genetrees_vs_species_tree)

    # Calculate mean RF distance: true vs species tree (noF)
    if "RF_true_genetrees_vs_species_tree_noF" in names(rep_data)
        noF_col = :RF_true_genetrees_vs_species_tree_noF
        df_summary[row_idx, col_rf_noF_all] =
            mean(skipmissing(rep_data.RF_true_genetrees_vs_species_tree_noF))
        df_summary[row_idx, col_rf_noF_loss] =
            safe_mean(loss_only.RF_true_genetrees_vs_species_tree_noF)
        df_summary[row_idx, col_rf_noF_dup_loss] =
            safe_mean(dup_and_loss.RF_true_genetrees_vs_species_tree_noF)
        df_summary[row_idx, col_rf_noF_nothing] =
            safe_mean(nothing_trees.RF_true_genetrees_vs_species_tree_noF)
        df_summary[row_idx, col_rf_noF_falseHP] =
            safe_mean(false_hp.RF_true_genetrees_vs_species_tree_noF)
        df_summary[row_idx, col_rf_noF_weakHP] =
            safe_mean(weak_hp.RF_true_genetrees_vs_species_tree_noF)
        df_summary[row_idx, col_rf_noF_strongHP] =
            safe_mean(strong_hp.RF_true_genetrees_vs_species_tree_noF)
    else
        # Fill with NaN if column doesn't exist
        df_summary[row_idx, col_rf_noF_all] = NaN
        df_summary[row_idx, col_rf_noF_loss] = NaN
        df_summary[row_idx, col_rf_noF_dup_loss] = NaN
        df_summary[row_idx, col_rf_noF_nothing] = NaN
        df_summary[row_idx, col_rf_noF_falseHP] = NaN
        df_summary[row_idx, col_rf_noF_weakHP] = NaN
        df_summary[row_idx, col_rf_noF_strongHP] = NaN
    end

    # Calculate mean RF distance: true vs estimated gene trees
    df_summary[row_idx, col_rf_est_all] =
        mean(rep_data.RF_true_vs_estimated_genetrees)
    df_summary[row_idx, col_rf_est_loss] =
        safe_mean(loss_only.RF_true_vs_estimated_genetrees)
    df_summary[row_idx, col_rf_est_dup_loss] =
        safe_mean(dup_and_loss.RF_true_vs_estimated_genetrees)
    df_summary[row_idx, col_rf_est_nothing] =
        safe_mean(nothing_trees.RF_true_vs_estimated_genetrees)
    df_summary[row_idx, col_rf_est_falseHP] =
        safe_mean(false_hp.RF_true_vs_estimated_genetrees)
    df_summary[row_idx, col_rf_est_weakHP] =
        safe_mean(weak_hp.RF_true_vs_estimated_genetrees)
    df_summary[row_idx, col_rf_est_strongHP] =
        safe_mean(strong_hp.RF_true_vs_estimated_genetrees)
end
...
function summarize_parameter_level_RF_statistics(paramname_root::String, 
    ...
    simulation_csv_file = joinpath(
        outfolder, "simulation_$(paramname_root).csv")
    ...
    df_param_summary = DataFrame(
        ...
        mean_internal_bl_locus_all = [
            mean(all_genes.avg_internal_branch_length_locus_tree)],
        mean_internal_bl_locus_loss_only = [
            safe_mean(loss_only.avg_internal_branch_length_locus_tree)],
        mean_internal_bl_locus_dup_and_loss = [
            safe_mean(dup_and_loss.avg_internal_branch_length_locus_tree)],
        mean_internal_bl_locus_nothing = [
            safe_mean(nothing_trees.avg_internal_branch_length_locus_tree)],
        mean_internal_bl_locus_false_HP = [
            safe_mean(false_hp.avg_internal_branch_length_locus_tree)],
        mean_internal_bl_locus_weak_HP = [
            safe_mean(weak_hp.avg_internal_branch_length_locus_tree)],
        mean_internal_bl_locus_strong_HP = [
            safe_mean(strong_hp.avg_internal_branch_length_locus_tree)],

        # Mean internal branch lengths - gene trees
        mean_internal_bl_gene_all = [
            mean(all_genes.avg_internal_branch_length_gene_tree)],
        mean_internal_bl_gene_loss_only = [
            safe_mean(loss_only.avg_internal_branch_length_gene_tree)],
        mean_internal_bl_gene_dup_and_loss = [
            safe_mean(dup_and_loss.avg_internal_branch_length_gene_tree)],
        mean_internal_bl_gene_nothing = [
            safe_mean(nothing_trees.avg_internal_branch_length_gene_tree)],
        mean_internal_bl_gene_false_HP = [
            safe_mean(false_hp.avg_internal_branch_length_gene_tree)],
        mean_internal_bl_gene_weak_HP = [
            safe_mean(weak_hp.avg_internal_branch_length_gene_tree)],
        mean_internal_bl_gene_strong_HP = [
            safe_mean(strong_hp.avg_internal_branch_length_gene_tree)],

        # Mean RF: true gene trees vs species tree
        mean_RF_genetree_vs_sptree_all = [
            mean(all_genes.RF_true_genetrees_vs_species_tree)],
        mean_RF_genetree_vs_sptree_loss_only = [
            safe_mean(loss_only.RF_true_genetrees_vs_species_tree)],
        mean_RF_genetree_vs_sptree_dup_and_loss = [
            safe_mean(dup_and_loss.RF_true_genetrees_vs_species_tree)],
        ...
        # Mean RF: true vs species tree (noF) - if column exists
        mean_RF_genetree_vs_sptree_noF_all = (
            "RF_true_genetrees_vs_species_tree_noF" in names(all_genes)
            ? [mean(skipmissing(
                all_genes.RF_true_genetrees_vs_species_tree_noF))]
            : [NaN]),
        mean_RF_genetree_vs_sptree_noF_loss_only = (
            "RF_true_genetrees_vs_species_tree_noF" in names(all_genes)
            ? [safe_mean(loss_only.RF_true_genetrees_vs_species_tree_noF)]
            : [NaN]),
        mean_RF_genetree_vs_sptree_noF_dup_and_loss = (
            "RF_true_genetrees_vs_species_tree_noF" in names(all_genes)
            ? [safe_mean(
                dup_and_loss.RF_true_genetrees_vs_species_tree_noF)]
            : [NaN]),
        mean_RF_genetree_vs_sptree_noF_nothing = (
            "RF_true_genetrees_vs_species_tree_noF" in names(all_genes)
            ? [safe_mean(
                nothing_trees.RF_true_genetrees_vs_species_tree_noF)]
            : [NaN]),
        mean_RF_genetree_vs_sptree_noF_false_HP = (
            "RF_true_genetrees_vs_species_tree_noF" in names(all_genes)
            ? [safe_mean(false_hp.RF_true_genetrees_vs_species_tree_noF)]
            : [NaN]),
        mean_RF_genetree_vs_sptree_noF_weak_HP = (
            "RF_true_genetrees_vs_species_tree_noF" in names(all_genes)
            ? [safe_mean(weak_hp.RF_true_genetrees_vs_species_tree_noF)]
            : [NaN]),
        mean_RF_genetree_vs_sptree_noF_strong_HP = (
            "RF_true_genetrees_vs_species_tree_noF" in names(all_genes)
            ? [safe_mean(strong_hp.RF_true_genetrees_vs_species_tree_noF)]
            : [NaN]),

        # Mean RF: true vs estimated gene trees
        mean_RF_true_vs_est_genetree_all = [
            mean(all_genes.RF_true_vs_estimated_genetrees)],
        mean_RF_true_vs_est_genetree_loss_only = [
            safe_mean(loss_only.RF_true_vs_estimated_genetrees)],
        mean_RF_true_vs_est_genetree_dup_and_loss = [
            safe_mean(dup_and_loss.RF_true_vs_estimated_genetrees)],
    ...
    # Read output of extract_summarize_simphy_summary, add RF columns
...
extract_simphy_summary:
    Extract and summarize statistics from simulation CSV for a parameter set.
    Calculates means across replicates and percentages for gene tree categories.

Inputs:
    paramname_root::String: Parameter set identifier
        (e.g., "DUP0.0001-LOS0.0001-RVL-N_ind1-SF1.0")
...
Outputs:
    ...
    - Percentages of trees experiencing different events
      (loss only, duplication+loss, nothing)
...
function extract_summarize_simphy_summary(paramname_root::String,
    ...
    num_trees_experiencing_gene_loss_only =
        df[!, :num_trees_experiencing_gene_loss_only]
    num_trees_experiencing_gene_duplication_and_loss =
        df[!, :num_trees_experiencing_gene_duplication_and_loss]
    num_trees_experiencing_nothing =
        df[!, :num_trees_experiencing_nothing]
    num_false_HP = "num_false_HP" in names(df) ?
        df[!, :num_false_HP] : zeros(nrow(df))
    num_weak_HP = "num_weak_HP" in names(df) ?
        df[!, :num_weak_HP] : zeros(nrow(df))
    num_strong_HP = "num_strong_HP" in names(df) ?
        df[!, :num_strong_HP] : zeros(nrow(df))
    ...
    num_estimated_species_tree_diff_from_truth =
        count(x -> !isnan(x) && x != 0, RF)
    ...
    df_summary = DataFrame(
        parameter_setting = [paramname_root],
        n_genes_mean = [mean(n_genes)],
        # find min/max n_genes across reps in the setting
        n_genes_min = [minimum(n_genes)],
        n_genes_max = [maximum(n_genes)],
        ...
        # Percentages of trees experiencing different events
        pert_trees_experiencing_gene_loss_only = [
            sum(num_trees_experiencing_gene_loss_only) /
            total_n_genes * 100],
        pert_trees_experiencing_gene_duplication_and_loss = [
            sum(num_trees_experiencing_gene_duplication_and_loss) /
            total_n_genes * 100],
        pert_trees_experiencing_nothing = [
            sum(num_trees_experiencing_nothing) / total_n_genes * 100],
...
There are a few senarioes for gene trees with hidden paralogy.
  Our definition: a gene going through hidden paralogy means
  simphy this gene goes through duplication and loss, and there
  are no multiple gene copies from the same accession observed
  in the gene tree.
  However, there are three other senarioes for hidden paralogy:

  False hidden paralogy:
  There are two senarioes for false HP:
    a. A gene got duplicated, but the duplicated gene copy quickly
    got lost. In this case, no hidden paralogy happened. However,
    technically gene loss and duplication did happen. For example,
    A_0 got copied into A_0 and A_1, and then A_1 got lost so that
    A_0 is left. In this case, I will call it "false" hidden
    paralogy. In this case, the suffixes of all the tips in the
    locus tree are the same, e.g. A_0, B_0, C_0, D_0, E_0, F_0.
    b. The suffixes of all the tips in the locus tree are not the
    same but the gene loss and duplication process doesn't change
    the topology or the branch length of the locus tree.
    In this case, we used distance method to figure out if the
    branch length of the locus tree changed vs the true sptree.

  Weak hidden paralogy:
    2. A gene got duplicated, and then the another copy of gene
    copy got lost. For example, A_0 got copied and became A_0 and
    A_1, and then the original gene copy A_0 got lost. However,
    the topology of the locus tree and species tree are the same.
    In this case, I will call it "weak" hidden paralogy.

  Strong HP:
    3. After the previous two senarioes, a gene tree has different
    topology from the species tree.
...
@everywhere begin 
  ...
  """
    function process_single_HP_line(line::String, 
        ...
        # Extract number from "rep01" -> 1
        simulation_rep = parse(Int, replace(rep_part, "rep" => ""))
        ...
        gene_id = parse(Int, replace(gene_part, "Gene" => "")) # number: 4
        int_part = split(split(gene_tree_name, "_")[5], ".")[1] # "Int6"
        ...
        simphy_raw_gene_tree_folder = setup_rep_output_folders(
            folder_path_list, simulation_rep, "genetrees_simphy")
        ...
        # read the corresponding locus tree
        locus_tree = readnewick(locus_trees[gene_id])
        ...
        for tip in tip_labels
            if startswith(tip, "Lost-") 
                ...
                # prune the lost tip from the locus tree
                deleteleaf!(locus_tree, tip)
                continue 
            else 
                # get the suffix after the last underscore
                suffix = split(tip, "_")[end]
                push!(suffixes, suffix)
            end
        end

        # First senario for false HP: check if all suffixes are the same
        if length(suffixes) == 1 # false hidden paralogy 
            return (line, "false", 0.0)

        else 
            true_sp_nwk = "(A:3440,((((B:880,C:880):1710,(D:930,E:930):1660):170,F:2760):180,(G:500,H:500):2440):500);"
            true_species_tree = readnewick(true_sp_nwk)
            # prune tips on locus tree (those starting with "lost-")
            ...
            # will be used later
            tiplabels_species = tipLabels(pruned_species_tree)
            sorted_tiplabels_species = sort(tiplabels_species) 

            if sorted_tiplabels_locus != sorted_tiplabels_species
                error("Tip labels differ between locus and " *
                      "species trees for $gene_tree_name " *
                      "in rep $simulation_rep")
            end 

            rf_distance = mudistance_semidirected(
                pruned_locus_tree, pruned_species_tree, preorder=true)

            if rf_distance == 0 
                # weak hidden paralogy or false hidden paralogy? 
                # rf_distance = 0 can still be false or weak hidden paralogy:
                # weak = locus branches changed vs species; false = unchanged.

                # Lineage variation scales species, not locus tree branches.
                ...
                if isapprox(sorted_m_locus, sorted_m_species;
                            atol=delta, rtol=0.0)
...
function postexamine_HP_based_on_locus_tree(
  ...
  fname = "Simphy_gene_duplication_and_loss_$paramname_root.csv"
  HP_file_path = joinpath(outfolder, fname)
  ...
  if isfile(HP_file_path)
      ...
      data_lines = hp_df[!, "gene_tree_file_path"]
  else
...
@everywhere begin
  ...
  function compare_astral_simphy_truespecies_1rep(
    ...
    if !isfile(astralfile)
        error("ASTRAL tree not found for rep $simulation_rep: $astralfile")
    end
    ...
    astraltre = readnewick(cleaned_tree_string)
    true_species = readnewick("(A,((((B,C),(D,E)),F),(G,H));")
    ...
    rf_btw_estimated_and_true_tree = mudistance_semidirected(
        pruned_true_species, pruned_astraltre, preorder=true)
    ...
    if n_true_species_tree != n_astral_tree
        error("Leaf count mismatch: true ($n_true_species_tree) " *
              "vs ASTRAL ($n_astral_tree) in rep $simulation_rep")
    end

    rf_btw_estimated_and_true_tree /= (2 * (n_true_species_tree - 3))

    # The true species tree VS. gene trees from simphy (true gene trees)
    simphyfolder = setup_rep_output_folders(
        folder_path_list, simulation_rep, "genetrees_singlecopy")
    astralfolder = setup_rep_output_folders(
        folder_path_list, simulation_rep, "astralfolder")
    ...
    if !isfile(mappingfile)
        error("Mapping file not found for rep $simulation_rep: $mappingfile")
    end
    if !isdir(simphyfolder)
        error("SimPhy folder not found for rep $simulation_rep: $simphyfolder")
    end
    ...
    if length(rf_result) == 3
        (gene_tree_files_with_paths,
         rf_btw_simphy_genetree_and_true_tree,
         rf_btw_simphy_genetree_and_true_tree_noF) = rf_result
    else
        gene_tree_files_with_paths,
        rf_btw_simphy_genetree_and_true_tree = rf_result
        rf_btw_simphy_genetree_and_true_tree_noF = Float64[]
    end

    return rf_btw_estimated_and_true_tree,
           gene_tree_files_with_paths,
           rf_btw_simphy_genetree_and_true_tree,
           rf_btw_simphy_genetree_and_true_tree_noF
  end
end
...
@everywhere begin
  ...
  Output:
    - Tuple{Vector{Float64}, Vector{String}}: A tuple containing:
      - RF distances between each true gene tree (SimPhy) and its corresponding 
      estimated gene tree (IQ-TREE) for the replicate
  ...
  function compare_gene_trees_iqtree_simphy_1rep(
    ...
    # Read mapping file: alignment files ↔ IQ-TREE trees
    ...
    for (i, gene_name) in enumerate(gene_names)
        ...
        if isfile(simphy_tree_file)
            try
                ...
                rf_distance = mudistance_semidirected(
                    simphy_tree, iqtree_tree, preorder=true)

                n_simphytree = length(tipLabels(simphy_tree))
                n_iqtreetree = length(tipLabels(iqtree_tree))
                ...
                if n_simphytree != n_iqtreetree
                    error("Leaf count mismatch: SimPhy ($n_simphytree) " *
                           "vs IQ-TREE ($n_iqtreetree) for $gene_name " *
                           "in rep $simulation_rep")
                end
                ...
                println("RF(norm) $gene_name: $normalized_rf_distance")

                push!(rf_distances, normalized_rf_distance)
                push!(gene_file_paths, simphy_tree_file)
            catch e
                println("Warning: Failed to parse $gene_name: $e")
...
@everywhere begin
    ...
    Calculate RF distances between SimPhy gene trees and the true species tree
    using individual-to-species mapping.

    This function reads SimPhy gene trees, maps individuals to species via
    the mapping file, and calculates RF distances between each mapped gene
    tree and the true species tree.

    # Arguments
    - `species_tree_string::String`: Newick string of the true species tree
    - `simphy_folder::String`: Path to folder with SimPhy gene tree files
    - `mapping_file::String`: Path to astral_mapping.txt file with
        individual-to-species mapping

    # Returns
    - `Vector{Float64}`: RF distances vs the true species tree

    # Notes
    - Gene tree files match `g_trees_noLocusID_Gene*.trees`
    - Mapping file format: `individual_name species_name` (space-separated)
    ...
    ```
    """
    function calculate_rf_distance_simphy_vs_species(
                                                species_tree_string::String,
                                                simphy_folder::String,
                                                mapping_file::String, 
                                                n_inds::Int,
        ...
        individual_to_species =
            create_mapping_file_based_on_gene_tree(mapping_file)

        rf_distances = Float64[]
        rf_distances_noF = Float64[]
        gene_tree_files_with_paths = Tuple{String, String}[]

        # Get all gene tree files (preserve original order)
        gene_tree_files = filter(
            f -> startswith(f, "g_trees_noLocusID_Gene") &&
                 endswith(f, ".trees"),
            readdir(simphy_folder))
        ...
        for gene_file in gene_tree_files
            ...
            if n_inds > 1 && !isempty(gene_tree_content) 
                ...
                subtree_normalized_rf_distances_noF = Float64[]

                gene_tree = readnewick(gene_tree_content)
                sisterdict = get_sisters_for_all_leafs(gene_tree)
                prune_related_leaves!(gene_tree, sisterdict)

                # can use simplify_tip_labels(pruned_tree) to simplify labels

                # For trees with duplicated species (e.g., "A_0" and "A_1"),
                # extract two sub-trees: one with "A_0", other with "A_1".
                # Done for all combinations of duplicated species.
                # With 8 taxa, at most 2^8 = 256 combinations,
                # but fewer since we pruned sister taxa first.

                species_dict = group_tips_by_species(gene_tree)
                # keys are species names, values are tip name vectors
                ...
                for combo in all_comb
                    ...
                    # modify tip labels: "A_0" → "A" etc.
                    mapped_subgene_tree = map_gene_tree_based_on_mapping_file(
                        subtree, individual_to_species)
                    ...
                    pruned_species_tree, pruned_mapped_subgene_tree =
                        prune_nonoverlapping_tips(species_tree,
                                                  mapped_subgene_tree)
                    ...
                    if remove_F_analysis
                        sp_noF, mg_noF =
                            prune_nonoverlapping_tips(species_tree_noF,
                                                      mapped_subgene_tree_noF)
                        pruned_species_tree_noF = sp_noF
                        pruned_mapped_subgene_tree_noF = mg_noF
                        rf_dist_noF = mudistance_semidirected(
                            sp_noF, mg_noF, preorder=true)
                    end
                    ...
                    if remove_F_analysis
                        ...
                        normalized_rf_dist_per_subtree_noF =
                            rf_dist_noF / (2 * (n_noF - 3))
                    end
                    ...
                    push!(subtree_normalized_rf_distances,
                          normalized_rf_dist_per_subtree)

                    if remove_F_analysis
                        push!(subtree_normalized_rf_distances_noF,
                              normalized_rf_dist_per_subtree_noF)
                    end
                    # stores all normalized RF distances for all subtrees
                    
                end 

                mean_rf_dist_for_one_gene_tree =
                    mean(subtree_normalized_rf_distances)
                push!(rf_distances, mean_rf_dist_for_one_gene_tree)
                push!(gene_tree_files_with_paths, (gene_file, gene_tree_path))

                if remove_F_analysis
                    mean_rf_dist_for_one_gene_tree_noF =
                        mean(subtree_normalized_rf_distances_noF)
            ...
            if n_inds == 1 && !isempty(gene_tree_content)
                ...
                no need to loop through sub-trees:
                map individuals to species directly =#
                mapped_gene_tree = map_gene_tree_based_on_mapping_file(
                    gene_tree, individual_to_species)
                ...
                if remove_F_analysis
                    pruned_species_tree_noF, pruned_mapped_gene_tree_noF =
                        prune_nonoverlapping_tips(species_tree_noF,
                                                  mapped_gene_tree_noF)
                    rf_dist_noF = mudistance_semidirected(
                        pruned_species_tree_noF,
...
Record the number of taxa in each gene/locus tree. The below function does: 
    1. count the number of taxa in each gene tree and locus tree 
        ...
        -> error out if gene tree and locus tree taxon counts differ
...
@everywhere begin 
    ...
    return: 
        ...
        2. num_taxa_gene_tree_list_per_rep: taxa count per gene tree
    ...
    function gene_and_locus_tree_taxa_and_internal_branch_stats(
        ...
        simphy_singlecopy_gene_tree_folder = setup_rep_output_folders(
            folder_path_list, simulation_rep, "genetrees_singlecopy")
        genetrees_list = readdir(simphy_singlecopy_gene_tree_folder)
        ...
        for genetree_filename in genetrees_list
            ...
            gene_tree_filepath = joinpath(
                simphy_singlecopy_gene_tree_folder, genetree_filename)
            push!(genetree_filepath_list_per_rep, gene_tree_filepath)

            # Extract int_id from the gene tree file name
            # "Int06" -> "06", "Gene06" -> "06"
            int_part = replace(
                split(split(genetree_filename, "_")[5], ".")[1], "Int" => "")
            gene_part = replace(
                split(split(genetree_filename, "_")[4], ".")[1], "Gene" => "")
            ...
            temp_dir = joinpath(simphy_raw_gene_tree_folder,
                                "Int$(int_id)", "1")
            if isdir(temp_dir)
                locus_tree_file = joinpath(temp_dir, "l_trees.trees")
            else
            ...
            gene_tree = readnewick(gene_tree_filepath)
            locus_tree = readnewick(readlines(locus_tree_file)[gene_id])

            to_remove = [t for t in tiplabels(locus_tree)
                          if startswith(t, "Lost")]
            for lost_tip in to_remove # only locus tree removes lost tips
            ...
            if num_taxa_gene_tree != num_taxa_locus_tree * n_inds
                error("Taxon count mismatch for gene $gene_tree_filepath")
            end 
            push!(num_taxa_gene_tree_list_per_rep, num_taxa_gene_tree)  

            # average internal branch length for this locus tree
            internal_branch_lengths_per_locus_tree = Float64[]
            # internal branch lengths for this gene tree
            ...
            for edge in locus_tree.edge # internal branches, locus tree
                if !isexternal(edge) 
            ...
            avg_internal_branch_length_locus_tree =
                mean(internal_branch_lengths_per_locus_tree)
            avg_internal_branch_length_gene_tree =
                mean(internal_branch_lengths_per_gene_tree)
...
Summarizes gene tree statistics by duplication/loss status
and hidden paralogy classification.
"""
function summarize_genetree_stats(
    ...
    if !isfile(df_internal_branch_length_stats)
        error("Required CSV not found: $df_internal_branch_length_stats")
    elseif !isfile(rf_true_genetrees_and_species_tree_path)
        error("Required CSV file not found: " *
              "$rf_true_genetrees_and_species_tree_path")
    elseif !isfile(rf_true_estimated_genetrees_stats_csv_path)
        error("Required CSV file not found: " *
              "$rf_true_estimated_genetrees_stats_csv_path")
    else
        df_stats = CSV.read(df_internal_branch_length_stats, DataFrame)
        df_rf_true_genetrees_and_species_tree = CSV.read(
            rf_true_genetrees_and_species_tree_path, DataFrame)
        df_rf_true_and_estimated_genetrees = CSV.read(
            rf_true_estimated_genetrees_stats_csv_path, DataFrame)
    end
    ...
    noF_col = "RF_true_genetrees_vs_species_tree_noF"
    if noF_col in names(df_rf_true_genetrees_and_species_tree)
    ...
    if !isfile(gene_duplication_loss_csv_path)
        error("Gene dup/loss CSV not found: $gene_duplication_loss_csv_path")
    else
        gene_duplication_loss_csv = CSV.read(
            gene_duplication_loss_csv_path, DataFrame)
        gene_duplication_and_loss_trees = Set{String}(
            gene_duplication_loss_csv[!, "gene_tree_file_path"])

        # find HP classifications
        filtered_false_HP = gene_duplication_loss_csv[
            gene_duplication_loss_csv[!, "false_HP"] .== "Y", :]
        false_HP_trees = Set{String}(
            filtered_false_HP[!, "gene_tree_file_path"])
        filtered_weak_HP = gene_duplication_loss_csv[
            gene_duplication_loss_csv[!, "weak_HP"] .== "Y", :]
        weak_HP_trees = Set{String}(filtered_weak_HP[!, "gene_tree_file_path"])
        filtered_strong_HP = gene_duplication_loss_csv[
            gene_duplication_loss_csv[!, "strong_HP"] .== "Y", :]
        strong_HP_trees = Set{String}(
            filtered_strong_HP[!, "gene_tree_file_path"])
    end 
    ...
    if !isfile(gene_loss_only_csv_path)
        ...
        gene_loss_only_trees = Set{String}(
            gene_loss_only_csv[!, "gene_tree_file_path"])
    end
    ...
    for i in 1:nrow(df_stats)
        ...
        gene_path_str = String(gene_path) # avoid SubString issues
...
function main()
    ...
    # Save RF scores between SimPhy gene trees and ground truth species tree
    println("Saving RF: SimPhy gene trees vs true species tree...")
    for simulation_rep in 1:n_reps
      rep_output_folder = setup_rep_output_folders(
          folder_path_list, simulation_rep, "")
    ...
    gene_tree_rf_results = pmap(
        simulation_rep -> compare_gene_trees_iqtree_simphy_1rep(
            simulation_rep, folder_path_list, rootfolder),
        1:n_reps)
    ...
    gene_duplication_and_loss_file = joinpath(outfolder,
        "Simphy_gene_duplication_and_loss_$paramname_root.csv")
    gene_loss_only_file = joinpath(outfolder,
        "Simphy_gene_loss_only_$paramname_root.csv")
    ...
    if isfile(gene_duplication_and_loss_file) 
        ...
        gene_duplication_and_loss_df = CSV.read(
            gene_duplication_and_loss_file, DataFrame)
        data_lines = gene_duplication_and_loss_df[!, "gene_tree_file_path"]
        # empty set when dup_rate == 0
        gene_duplication_and_loss_trees =
            Set{String}(filter(!isempty, data_lines))
    else
        error("Dup/loss gene tree file not found: " *
              "$gene_duplication_and_loss_file")
    end 
    
    if isfile(gene_loss_only_file)  
        gene_loss_only_df = CSV.read(gene_loss_only_file, DataFrame)
        data_lines = gene_loss_only_df[!, "gene_tree_file_path"]
    ...
    false_HP_paths, weak_HP_paths, strong_HP_paths =
        postexamine_HP_based_on_locus_tree(
    ...
    if isfile(gene_duplication_and_loss_file)
        ...
        println("Updated dup/loss CSV with HP classification")
    else
        println("Warning: Could not update CSV - not found: " *
                "$gene_duplication_and_loss_file")
    end
    ...
    # -----------------------------------------------# 
    println("="^30)
    println("Calculating RF dist: SimPhy gene trees vs true species tree...")
    ...
    # Write SimPhy vs species tree RF summary to CSV
    ...
    if isfile(csv_file)
        ...
        gene_duplication_loss_csv_file = joinpath(outfolder,
            "Simphy_gene_duplication_and_loss_$paramname_root.csv")
        ...
        for rep_id in 1:n_reps
            if rep_id <= nrow(df_sim)
                ...
                # Build a boolean mask robust to Missing values
                rep_mask = [(!ismissing(path) &&
                             occursin("/rep$rep_string/", path))
                            for path in df_rf.gene_tree_file_path]
                rep_data = df_rf[rep_mask, :]

                # Count HP categories, guarding against Missing
                is_Y = x -> (!ismissing(x) && x == "Y")
                num_false = count(is_Y, rep_data.false_HP)
                num_weak  = count(is_Y, rep_data.weak_HP)
                num_strong = count(is_Y, rep_data.strong_HP)
    ...
    for simulation_rep in 1:n_reps
        ...
        avg_internal_branch_length_gene_trees =
            gene_and_locus_tree_taxa_and_internal_branch_stats(
            simulation_rep,
            folder_path_list,
            rootfolder,
        ...
        append!(all_avg_internal_branch_length_locus_trees,
                avg_internal_branch_length_locus_trees)
        append!(all_avg_internal_branch_length_gene_trees,
                avg_internal_branch_length_gene_trees)
    end
    # Write to CSV
    ...
    df_taxa_internal = DataFrame(
        ...
        avg_internal_branch_length_locus_tree =
            all_avg_internal_branch_length_locus_trees,
        avg_internal_branch_length_gene_tree =
            all_avg_internal_branch_length_gene_trees
    )
    ...
    #-----------------------------------------------# 
    println("="^30)
    println("Postprocessing completed for: $paramname_root")
