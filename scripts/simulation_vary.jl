...
rootfolder = pwd()
paramname_root = set_up_paramname_root(dup_rate, loss_rate, ratevar, n_inds, SF) 
...
if occursin("L", ratevar) # add tree with variation across lineages
  ...
  species_tree = "(A:3440,((((B:480,C:480):1710,(D:530,E:530):1660):170,F:2360):180,(G:100,H:100):2440):900);"
  parameters *= "-s $species_tree\n"
...
Inputs: 
    max_iteration_simphy: Maximum number of iterations for SimPhy (default = 100).
...
Outputs:
    ...
    gene_trees_gene_loss_and_duplication_1rep: 
      List of gene trees experiencing gene loss and duplication.
  ...
  function run_simulation_1rep(
    ...
    while iteration < max_iteration_simphy
      ...
      if num < n_genes 
        ...
        println("Rep$simulation_rep Iter$iteration: " * 
    ...
    if length(gene_tree_files) > n_genes
      ...
      for file_to_remove in files_to_remove
        ...
        simphy_tree_file_path = joinpath(simphyfolder, "Int$int_id", "1")
        mapsl_file_to_remove = joinpath(simphy_tree_file_path, "$(gene_id).mapsl")  
        ...
        # permanently remove those files
      ...
      println("Rep$simulation_rep: Limited to first $n_genes gene trees") 
      println("Removed $(length(files_to_remove)) excess trees")
    end

    # filter out NaN values before calculating the mean 
    # This step is important or otherwise the output mean would have lots of NaN
    valid_values = filter(!isnan, avg_num_leaf_left_in_trees_one_rep)
    average_num_leaf_left_in_trees_for_1rep = isempty(valid_values) ? NaN : mean(valid_values)
...
function main() 
  ...
  if !debug_mode
      println("Debugging mode is OFF.")
      println("Removing temporary seqgen nexus and phylip folders and iqtree gene.treefile to save space...")
