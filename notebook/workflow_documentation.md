# Documentation for the overall workflow for each script 

# The major script to simulate genes and molecular sequences, inferring gene trees and species tree 
Section 1: simulation_iqtree.jl 
Section 2: snaq.jl 
Section 3: findgraph.jl 
Section 4: Seed generators 

# Section 1: simulation_iqtree.jl  
## Documentation of how to simulate gene trees using SimPhy 

Simphy master configuration file is samed in simphy-configs/ (Hi ChatGPT, please change this to a folder link in the repo), and we then added parameters based on specific parameter setting specified in simulation_iqtree.jl to make specific simphy configuration file individually. It is noted that rs (number of replicate) = 1 in the master simphy configuration file (reasons seen below). 

To simulate hidden paralogy, meaning that duplicated gene copy is lost during evolution, we deployed the following approach. If the simulated gene trees has: 
  * more than one repeated gene copy: 
    Then, it is obvious that gene duplication happened, so paralogy is not hidden. This tree will be removed from downstream analysis. 
  * Some gene loss events and there are <= 3 taxa left: 
    Not phylogenetically informative and this tree will be removed from downstream analysis. 
  * 0 or 1 gene copy per individual, and >= 4 taxa left: 
    If there is paralogy, then it's hidden. The associated gene tree file is the same as the original, except for taxon names. Gene copy id is removed, the taxon name still contains the species + individual number.
  * If dup_rate = 0, then all gene copies are orthologous, so there is no hidden paralogy. The below code will only change the tip names by removing gene copy id. 
  ...
  * This newick string modification might remove some trees from the pipeline. 
    ...
    c. If iteration > max_iteration_simphy, num of gene tress < n_genes_min 
      -> pipeline continues but will write a warning.log
    The info about which rep generates how many gene trees is output to simphy_simulation.csv

In simulation_iqtree.jl, we run SimPhy simulation based on a []
...
# Documentation for generation time and effective population size used in Simphy simulation: 
Because the tree span across reptiles, birds, and mannals, we find an estimated average generation tiem used in this study. 

| Vertebrate Group | Generation Time (years)          | Effective Population Size (N<sub>e</sub>) | Citation |
|------------------|----------------------------------|-------------------------------------------|----------|
| Mammals          | ~5 years (6.6 from Pacifici et al. 2013) | ~10,000 (10⁴)                             | Pacifici et al., 2013; Li & Durbin, 2011 |
| Birds            | ~3 years                         | ~100,000 (10⁵)                            | Bird et al., 2020; Balakrishnan & Edwards, 2009 |
| Reptiles         | ~10 years (varies 1–20+ yr)      | ~10,000 (10⁴) (varies widely)             | Wilkinson et al., 2008; FitzSimmons et al., 1997 |
| Amphibians       | ~4 years                         | ~1,000 (10³)                              | Wells, 2007; Newman & Squire, 2001 |

Reference for effective population size and generation time: 
Balakrishnan, C. N., & Edwards, S. V. (2009). Nucleotide variation, linkage disequilibrium and founder-facilitated speciation in wild populations of the zebra finch (Taeniopygia guttata). Genetics, 181(2), 645–660. https://doi.org/10.1534/genetics.108.096719

Bird, J. P., Martin, R., Akçakaya, H. R., Gilroy, J., Burfield, I. J., Garnett, S. T., ... & Butchart, S. H. M. (2020). Generation lengths of the world’s birds and their implications for extinction risk. Conservation Biology, 34(5), 1252–1261. https://doi.org/10.1111/cobi.13486

FitzSimmons, N. N., Limpus, C. J., Norman, J. A., Goldizen, A. R., Miller, J. D., & Moritz, C. (1997). Philopatry of male marine turtles inferred from mitochondrial DNA markers. Proceedings of the National Academy of Sciences, 94(16), 8912–8917. https://doi.org/10.1073/pnas.94.16.8912
...
Newman, R. A., & Squire, T. (2001). Microsatellite variation and fine-scale population structure in the wood frog (Rana sylvatica). Molecular Ecology, 10(5), 1087–1100. https://doi.org/10.1046/j.1365-294X.2001.01254.x

Pacifici, M., Santini, L., Di Marco, M., Baisero, D., Francucci, L., Grottolo Marasini, G., ... & Rondinini, C. (2013). Generation length for mammals. Nature Conservation, 5, 87–94. https://doi.org/10.3897/natureconservation.5.5734

    Pacifici uses 5427 mannal species time and find that average GT for mammals is 6.6 years from ~0.17 years (e.g., small rodents) to over 90 years (e.g., whales and elephants). 

Wells, K. D. (2007). The ecology and behavior of amphibians. University of Chicago Press.

Wilkinson, A., Müller, H., & Huber, L. (2008). Understanding of causality in trained pigeons. Animal Cognition, 11, 349–357. https://doi.org/10.1007/s10071-007-0122-5 
